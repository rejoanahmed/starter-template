import ActionSheetThemed from "@propia/components/ActionSheetThemed";
...
import PageLoader from "@propia/components/PageLoader";
...
export default function ChatDetailScreen() {
  ...
  const { id: _ } = useLocalSearchParams();
  ...
  const renderMessage = ({ item }: { item: Message }) => (
    ...
    >
      ...
      >
        <ThemedText className={item.isSent ? "text-white" : ""}>
          {item.text}
        </ThemedText>
        <ThemedText
          className={`text-xs mt-1 ${item.isSent ? "text-white/70" : "text-light-subtext dark:text-dark-subtext"}`}
        >
          {item.timestamp}
        </ThemedText>
      </View>
      ...
      >
        ...
        >
          <View className="p-4">
            ...
            <TouchableOpacity className="py-4 flex-row items-center">
              <Icon className="mr-3" name="Slash" size={20} />
